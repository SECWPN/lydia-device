...
REF_DEFAULT="v0.1.0"   # you will update this when you cut releases
...
usage() {
  cat <<EOF
Usage: install.sh [--ref vX.Y.Z] [--sha COMMIT] [--serial /dev/ttyUSB0] [--hz 2.0] [--repo URL]
EOF
}
...
install_uv() {
  ...
  UV_INSTALL_DIR="/usr/local/bin" curl -fsSL https://astral.sh/uv/install.sh | sh
  require_cmd uv
}
